...
import static org.firstinspires.ftc.teamcode.opmode.TeleOp.TeleOpQualifier.blockageblockposition;
import static org.firstinspires.ftc.teamcode.pedroPathing.TeleOpQualifier.blockagereleaseposition;
...
@Autonomous(name = "Blue Far Twelve HOME HOME HOME HOME V1 Red9short")
//three preload and take home twice , parking

public class Blue12farAllHomeTurret extends LinearOpMode {
...
//    IntakeSubsystem intakeSubsystem;
    ...
    private final Pose startPose = new Pose(96,6.25, Math.toRadians(0)).mirror();
    private final Pose startPoseCP = new Pose(96,11.25, Math.toRadians(0)).mirror(); // Start Pose further zone of our robot.
    private final Pose shootPose = new Pose(96, 12.25, Math.toRadians(68)).mirror(); //70-73 65-75 Scoring Pose of our robot. It is facing the goal at a 115 degree angle.
    private final Pose readyFirstPickupPose = new Pose(92, 36.25, Math.toRadians(0)).mirror();
    private final Pose readyFirstHomePickupPose = new Pose(120, 12.25, Math.toRadians(0)).mirror();
    private final Pose readySecondHomePickupPose = new Pose(120, 12.25, Math.toRadians(0)).mirror();
    private final Pose readyThirdHomePickupPose = new Pose(120, 12.25, Math.toRadians(0)).mirror();
    private final Pose readySecondHomeSecondTryPickupPose = new Pose(120, 16.25, Math.toRadians(0)).mirror();
    private final Pose readyThirdHomeSecondTryPickupPose = new Pose(120, 16.25, Math.toRadians(0)).mirror();
    private final Pose firstPickupPose = new Pose(134, 36.25, Math.toRadians(0)).mirror(); // //i.Near (audience side): GPP of Artifacts from the Spike Mark.
    private final Pose firstHomePickupPose = new Pose(139, 6.25, Math.toRadians(0)).mirror();
    private final Pose secondHomePickupPose = new Pose(139, 6.25, Math.toRadians(0)).mirror();
    private final Pose thirdHomePickupPose = new Pose(139, 6.25, Math.toRadians(0)).mirror();
    private final Pose secondHomeSecondTryPickupPose = new Pose(136, 16.25, Math.toRadians(0)).mirror();
    private final Pose thirdHomeSecondTryPickupPose = new Pose(136, 16.25, Math.toRadians(0)).mirror();
...
//                robot.greenLED = RevBlinkinLedDriver.BlinkinPattern.GREEN;
    ...
    private void buildPaths() {
        ...
        driveFirstHomePickup = follower.pathBuilder()
                ...
                .build();
        driveFirstHomePickupShoot = follower.pathBuilder()
                .addPath(new BezierLine(firstHomePickupPose, shootPose))
